...
function calc() {
  var persons = document.querySelectorAll('.counter-block-input')[0],
      // здесь получаем первый элемент и поэтому указываем что это нулевой.
  ...
  persons.addEventListener('input', function () {
    ...
    if (restDays.value == '') {
      //будем проверять что польщователь количество дней пустым не оставил, а если оставит то в общей сумме будет 0
      totalValue.innerHTML = 0;
  ...
  restDays.addEventListener('input', function () {
    ...
    if (persons.value == '') {
      //будем проверять что пользователь количество дней пустым не оставил, а если оставит то в общей сумме будет 0
      totalValue.innerHTML = 0;
  ...
  place.addEventListener('input', function () {
    if (persons.value == '' || restDays.value == '') {
      //будем проверять заполнены ли наши поля то все равно будем написать 0
      totalValue.innerHTML = 0;
    } else {
      //в ином случае будем записать новую функцию
      var a = total; // чтобы избежать потеря данных, мы создаем техническое переменное а то при изменение база   данных наш тотал может не показать то значение
...
function slider() {
  ...
  function showSlides(n) {
    if (n > slides.length) {
      // если n больше количество всех слайдов то есть мы перелистали все фотки, то аргумент функции равно одному то есть первому,
      slideIndex = 1;
    }
  ...
  dotsWrap.addEventListener('click', function (event) {
    for (var i = 0; i < dots.length + 1; i++) {
      // здесь мы запускаем цикл на один раз больше
...
function timer() {
  var deadline = '2022-1-10';

  function getTimeRemaining(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        seconds = Math.floor(t / 1000 % 60),
        minutes = Math.floor(t / 1000 / 60 % 60),
        hours = Math.floor(t / (1000 * 60 * 60)); // hours = Math.floor((t/1000/60/60) % 24);  чтобы еще добавить дни  в таймер.
...
__webpack_require__(/*! formdata-polyfill */ "./node_modules/formdata-polyfill/formdata.min.js"); // that's it   
